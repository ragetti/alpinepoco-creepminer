node {
	...
	stage('Run') {
		...
		docker.image("${env.JOB_NAME}:${env.BUILD_ID}").run('--name ${env.JOB_NAME} -p 8126:8126 --restart=on-failure --entrypoint="/app/creepMiner" -v "/home/miner/dockerfiles/mycreepminer/mining.conf:/app/mining.conf" -v "/mnt/plots:/plots/:ro" ')
/*		
		docker.withRegistry('http://192.168.1.99:5000', 'private-hub-credentials') {
